...
def predict_outputs():
    ...
    for model_run_instance in model_instances:
        for experiment in Settings.COLUMNS.GENERALIZABILITY_FILTERS:
            print("\n\nPredicting for label: {}".format(experiment["experiment_id"]))
            ...
            sentence_column = experiment["SENTENCE_COLUMN"]
            input_data = [{ 'id': input_example[ID_column], 'sentence': input_example[sentence_column] } for _, input_example in X.iterrows()]
            for model_info in models:
                fold_num = model_info['fold']
                print("\n\nRunning predictions for fold #{}...".format(fold_num))
                ...
                # print out predictions per fold
                predictions = pd.merge(pd.DataFrame(input_data), pd.DataFrame(predicted_probabilities_for_fold, columns=['0_probs', '1_probs']), left_index=True, right_index=True)

                Print.output_csv(OutputFolders.predictions.name,
                             ['id', 'sentence', '0_probs', '1_probs'],
