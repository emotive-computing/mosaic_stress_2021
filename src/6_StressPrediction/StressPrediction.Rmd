...
p = p + scale_fill_discrete(breaks=c("True_value","Predicted_value"),labels=c("True Value","Predicted Value")) + scale_colour_discrete(breaks=c("True_value","Predicted_value"),labels=c("True Value","Predicted Value"))
p
ggsave("lr_rf_gt_densities_smape.pdf", height=3, width=6)
```
...
data.smape.sub <- ddply(data.smape, c("snapshot_id", "Model", "Label"), summarize, n = length(True_value), true.mn = mean(True_value, na.rm = T), pred.mn = mean(Predicted_value, na.rm = T), smape = Metrics::smape(True_value, Predicted_value), rho = cor(True_value, Predicted_value, method = "spearman", use = "pairwise.complete.obs"));
...
# SMAPE histograms
p = ggplot(data.smape.sub, aes(x = smape, fill = Label)) + geom_histogram(binwidth = .02) + geom_vline(xintercept = mean(subset(data.sub, Label == "stress.d")$smape, na.rm = T), color = "red") + geom_vline(xintercept = mean(subset(data.sub, Label == "stress.d_shuffled")$smape, na.rm = T), color = "green") + geom_vline(xintercept = mean(subset(data.sub, Label == "stress.d_shuffledWithinSubject")$smape, na.rm = T), color = "blue")
p = p + theme_light()  + theme(legend.position = c(0.627, 0.8), legend.background = element_rect(fill="transparent"), panel.background = element_blank(), legend.title=element_blank(), strip.background = element_rect(fill="white"), strip.text = element_text(color="black", face="bold", size=13, hjust=0.5), axis.text.x = element_text(size=12), axis.text.y = element_text(size=13), text = element_text(size=13))
Model.labs <- c("Elastic Net","Random Forest")
names(Model.labs) <- c("ElasticNet", "RandomForestRegressor")
p = p + facet_wrap(~Model, labeller=labeller(Model=Model.labs))
