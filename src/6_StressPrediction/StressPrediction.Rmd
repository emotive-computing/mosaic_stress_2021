...
p
ggsave("lr_rf_gt_densities.pdf", height=3, width=6)
```
...
Metrics::smape(subset(data.melt.stress.lr, variable == "True_value")$value, subset(data.melt.stress.lr, variable == "Predicted_value")$value)
...
Metrics::smape(subset(data.melt.shuffled.cross.lr, variable == "True_value")$value, subset(data.melt.shuffled.cross.lr, variable == "Predicted_value")$value)
...
Metrics::smape(subset(data.melt.shuffled.within.lr, variable == "True_value")$value, subset(data.melt.shuffled.within.lr, variable == "Predicted_value")$value)
...
Metrics::smape(subset(data.melt.stress.rf, variable == "True_value")$value, subset(data.melt.stress.rf, variable == "Predicted_value")$value)
...
Metrics::smape(subset(data.melt.shuffled.cross.rf, variable == "True_value")$value, subset(data.melt.shuffled.cross.rf, variable == "Predicted_value")$value)
...
Metrics::smape(subset(data.melt.shuffled.within.rf, variable == "True_value")$value, subset(data.melt.shuffled.within.rf, variable == "Predicted_value")$value)
...
```{r, warning = F}
data.sub <- ddply(data, c("snapshot_id", "Model", "Label"), summarize, n = length(True_value), true.mn = mean(True_value, na.rm = T), pred.mn = mean(Predicted_value, na.rm = T), smape = Metrics::smape(True_value, Predicted_value), rho = cor(True_value, Predicted_value, method = "spearman", use = "pairwise.complete.obs"));
...
# p_compare$p.format[p_compare$p.adj < 0.001] = "p<0.001"
# p_compare$p.format[p_compare$p.adj >= 0.001] = paste0("p=",p_compare$p.format[p_compare$p.adj >= 0.001])
p = ggplot(data.sub, aes(y = smape, x = Label)) + geom_boxplot(aes(fill=Label), width=0.6, outlier.shape=NA) + geom_hline(yintercept = 0, linetype="dashed")
p = p + theme_light()  + theme(legend.position = "none", panel.background = element_blank(), strip.background = element_rect(fill="white"), strip.text = element_text(color="black", face="bold", size=13, hjust=0.5), axis.text.x = element_text(size=12), axis.text.y = element_text(size=13), text = element_text(size=13))
Model.labs <- c("Elastic Net","Random Forest")
names(Model.labs) <- c("ElasticNet", "RandomForestRegressor")
p = p + facet_wrap(~Model, labeller=labeller(Model=Model.labs))
...
p = p + theme_light()  + theme(legend.position = c(0.627, 0.8), legend.background = element_rect(fill="transparent"), panel.background = element_blank(), legend.title=element_blank(), strip.background = element_rect(fill="white"), strip.text = element_text(color="black", face="bold", size=13, hjust=0.5), axis.text.x = element_text(size=12), axis.text.y = element_text(size=13), text = element_text(size=13))
Model.labs <- c("Elastic Net","Random Forest")
names(Model.labs) <- c("ElasticNet", "RandomForestRegressor")
p = p + facet_wrap(~Model, labeller=labeller(Model=Model.labs))
...
t.test(data.sub.cast$smape_ElasticNet_stress.d, data.sub.cast$smape_ElasticNet_stress.d_shuffled, paired = T)

t.test(data.sub.cast$smape_ElasticNet_stress.d, data.sub.cast$smape_ElasticNet_stress.d_shuffledWithinSubject, paired = T)

t.test(data.sub.cast$smape_RandomForestRegressor_stress.d, data.sub.cast$smape_RandomForestRegressor_stress.d_shuffled, paired = T)

t.test(data.sub.cast$smape_RandomForestRegressor_stress.d, data.sub.cast$smape_RandomForestRegressor_stress.d_shuffledWithinSubject, paired = T)
...
t.test(data.sub.cast$smape_ElasticNet_stress.d, data.sub.cast$smape_RandomForestRegressor_stress.d, paired = T)
...
#Compare LR rho to rho from Garmin stress and HRV
```{r, warning = F}
...
data.comp <- ddply(data.actual.rf, c("snapshot_id", "Model"), summarize, rho.model = cor(True_value, Predicted_value, method = "spearman", use = "pairwise.complete.obs"), rho.garmin = cor(True_value, avg_garmin_stress, method = "spearman", use = "pairwise.complete.obs"));
...
data.comp.hrv.melt$Model <- "RandomForestRegressor"
...
t.test(subset(data.comp.melt, Model=="RandomForestRegressor" & variable=="rho.model")$value, mu=0, alternative = "two.sided")
t.test(subset(data.comp.melt, Model=="RandomForestRegressor" & variable=="rho.garmin")$value, mu=0, alternative = "two.sided")
t.test(subset(data.comp.melt, Model=="RandomForestRegressor" & variable=="rho.hrv")$value, mu=0, alternative = "two.sided")
...
p = p + stat_pvalue_manual(data=p_compare, label="p.format", xmin="group1", xmax="group2", y.position = "y.position")
p
ggsave("within_subj_spearman_stress_compare.pdf", height=3, width=5)
...
ggplot(data.comp.melt, aes(x = value, fill = variable))+ geom_histogram(binwidth = .05) + geom_vline(xintercept = mean(data.comp$rho.model, na.rm = T), color = "red") + geom_vline(xintercept = mean(data.comp$rho.garmin, na.rm = T), color= "green") + geom_vline(xintercept = mean(data.comp$rho.hrv, na.rm = T), color = "blue")
...
#IGTBs to show that the model accuracy are mostly independent of demographics
```{r, warning = F}
...
mt <- lm(true.mn ~ n + age + gender + lang  + educRcd + incomeRcd + supervise + cohortRcd - 1, data = data.merged.rf)
#m1 <- lm(rho ~ n + age + gender + lang  + educRcd + incomeRcd + supervise + cohortRcd, data = data.merged.rf)
m1 <- lm(pred.mn ~ n + age + gender + lang  + educRcd + incomeRcd + supervise + cohortRcd - 1, data = data.merged.rf)
m2 <- lm(rho ~ n + age + gender + lang  + educRcd + incomeRcd + supervise + cohortRcd - 1, data = data.merged.rf)
...
data.merged.rf.igtb <- data.merged.lr[complete.cases(data.merged.lr[,rf_cols]),rf_cols]
rf_weights = list()
pred_perf = list()
for (i in 1:1000) {
